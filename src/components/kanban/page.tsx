...
export default function KanbanPage() {
  ...
  const handleTaskMove = async (taskId: string, newStatus: 'todo' | 'doing' | 'done') => {
    if (user?.type !== 'programmer') return;
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/tasks/${taskId}`, {
        ...
        body: JSON.stringify({
          ...
          ...(newStatus === 'doing' && { real_start_date: new Date().toISOString() }),
          ...(newStatus === 'done' && { real_end_date: new Date().toISOString() }),
        }),
      });
  ...
  return (
    <MainLayout user={user}>
      <div className="mb-6">
        ...
        <div className="flex gap-4 mt-4 text-sm">
          ...
          <div className="bg-red-50 px-3 py-2 rounded-lg">
            ...
            <span className="text-red-600">{tasks.filter(t => t.status === 'todo').length}</span>
          </div>
          <div className="bg-yellow-50 px-3 py-2 rounded-lg">
            <span className="font-semibold text-yellow-800">Em Progresso: </span>
            <span className="text-yellow-600">{tasks.filter(t => t.status === 'doing').length}</span>
          </div>
          <div className="bg-green-50 px-3 py-2 rounded-lg">
            <span className="font-semibold text-green-800">Concluído: </span>
            <span className="text-green-600">{tasks.filter(t => t.status === 'done').length}</span>
          </div>
        </div>
      </div>
      ...
      <TaskDetails
        ...
        userType={user.type}
        onEditTask={user.type === 'manager' ? handleEditTask : undefined}
      />
    </MainLayout>
  );
}